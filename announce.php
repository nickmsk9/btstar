...
if(isset($_GET['info_hash'])) $info_hash = $_GET['info_hash'];
if(isset($_GET['peer_id'])) $peer_id = $_GET['peer_id'];
if(isset($_GET['event'])) $event = $_GET['event'];
//if(isset($_GET['ip'])) $ip = $_GET['ip'];
if(isset($_GET['localip'])) $localip = $_GET['localip'];
$port = 0 + $_GET['port'];
$downloaded = 0 + $_GET['downloaded'];
$uploaded = 0 + $_GET['uploaded'];
$left = 0 + $_GET['left'];
...
if ($i=strpos($passkey, '?')) {
	$tmp = substr($passkey, $i);
	$passkey = substr($passkey, 0, $i);
	$GLOBALS[substr($tmp, 1, strpos($tmp, '=')-1)] = substr($tmp, strpos($tmp, '=')+1);
}
	...
	if (!isset($x)) err('Missing key: '.$x);
		foreach (array('info_hash','peer_id') as $x)
			...
			if (strlen($passkey) != 32)
				err('Invalid passkey or multitracker torrent');
$ip = getip();
$rsize = 50;
...
if(substr($peer_id, 1, 2) == 'BC' && substr($peer_id, 5, 2) != 70 && substr($peer_id, 5, 2) == 63 && substr($peer_id, 5, 2) != 77 && substr($peer_id, 5, 2) >= 59/* && substr($peer_id, 5, 2) <= 88*/) err("BitComet ".substr($peer_id, 5, 2)." is banned.");
if(ereg("^ABC\\/ABC", $agent)) err("ABC is Banned.");
if(ereg("^0P3R4H", $agent)) err("IBrowser Opera is not a cool BT client.");
if(ereg("^AzureusBitTyrant 2.5.0.0", $agent)) err("AzureusBitTyrant 2.5.0.0 is Banned");
if(ereg("^BTuga/Revolution-2.6", $agent)) err("BTuga/Revolution-2.6 is Banned");
if(ereg("^Azureus 2.4.0.2", $agent)) err("Azureus 2.4.0.2 is Banned");
if(ereg("^Azureus 2.5.0.0", $agent)) err("Azureus 2.5.0.0 is Banned");
if(ereg("^Azureus 2.5.0.2", $agent)) err("Azureus 2.5.0.2 is Banned");
if(ereg("^BitTorrent/3.4.2", $agent)) err("BitTorrent/3.4.2 is Banned");
if(ereg("^Mozilla/4.0 ", $agent)) err("Mozilla/4.0 is Banned");
...
$res = mysql_query('SELECT id, banned, free, seeders + leechers AS numpeers, multitracker, UNIX_TIMESTAMP(added) AS ts FROM torrents WHERE info_hash = "'.$hash.'"') or err(mysql_error());
$torrent = mysql_fetch_array($res);
if (!$torrent)
	err('Torrent not registered with this tracker.');
if ($torrent['multitracker']==1)
	err('This is multitracker torrent!');
...
unset($self);   // НАВЕРНОЕ МОЖНО УБРАТЬ?
...
if (!isset($self)) {
	...
	if ($upthis > 0 || $downthis > 0) {
		if($torrent['free']>0){
        $advdown = $downthis*((100-$torrent['free'])/100);
        }
		} 
		mysql_query('UPDATE LOW_PRIORITY users SET uploaded = uploaded + '.$advup.', downloaded = downloaded + '.$advdown.' WHERE id='.$userid) or err('Tracker error 3');
}
//$first = timer() - $now;
$dt = "'".date('Y-m-d H:i:s')."'";
$updateset = array();
...
if ($event == 'stopped') {
	...
	if (isset($self)) {
		...
		//$first = timer() - $now;
		if (!mysql_fetch_array(mysql_query('SELECT id FROM snatched WHERE torrent = '.$torrentid.' AND userid = '.$userid) or err(mysql_error())))
		...
		if (mysql_query("INSERT LOW_PRIORITY INTO peers (connectable, torrent, peer_id, ip, port, uploaded, downloaded, to_go, started, last_action, seeder, userid, agent, uploadoffset, downloadoffset, passkey) VALUES ('$connectable', $torrentid, " . sqlesc($peer_id) . ", '$ip', $port, $uploaded, $downloaded, $left, NOW(), NOW(), '$seeder', $userid, " . sqlesc($agent) . ", $uploaded, $downloaded, '$passkey')")) {
			if ($seeder == 'yes')
				$updateset[] = 'seeders = seeders + 1';
			else
				$updateset[] = 'leechers = leechers + 1';
...
if (count($updateset))
	mysql_query('UPDATE LOW_PRIORITY torrents SET ' . implode(", ", $updateset) . ' WHERE id = '.$torrentid);

if (count($snatch_updateset))
	mysql_query('UPDATE LOW_PRIORITY snatched SET ' . implode(", ", $snatch_updateset) . ' WHERE torrent = '.$torrentid.' AND userid = '.$userid) or err(mysql_error()."Line: ".__LINE__);

if ($_SERVER["HTTP_ACCEPT_ENCODING"] == "gzip") {
	header("Content-Encoding: gzip");
	echo gzencode(benc_resp_raw($resp), 3, FORCE_GZIP);
} else
	echo benc_resp_raw($resp);
/*$fh=fopen('logs/announce.'.date("Y-m-d").'.log','a+');
fwrite($fh,"\n".(timer()-$now).' secs, first: '.$first.', event: '.$event);
fclose($fh);*/
?>
