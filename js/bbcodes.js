...
{
    var FoundErrors = '';
    var enterTEXT   = prompt(text_enter_spoiler);
    var enterTITLE = prompt(text_enter_spoiler_name);

    if (!enterTEXT) {
        FoundErrors += " " + error_no_spoiler;
    }
    if (!enterTITLE) {
        enterTITLE = 'Ñêðûòàÿ èíôîðìàöèÿ';
    }
	...
	doInsert("[spoiler="+enterTITLE+"]"+enterTEXT+"[/spoiler]", "", false);
}
