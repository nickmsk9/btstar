...
function get_user_class_color($class, $username)
{
  ...
  {
    case UC_GOD:
	  return "<span style=\"color:#7BA813\" title=\"Создатель трекера\"><b>" . $username . "</b></span>";
	  break;
    case UC_SYSOP:
    ...
    case UC_MODERATOR:
     ...
     case UC_USER:
      ...
      break;
	 case UC_BOT:
	  return "<span style=\"color:#ddd\" title=\"Бот\">".$username."</span>";
	  break;
  }
  return "$username";
}
...
{
	return preg_replace(
    "/(<a href=redir\.php\?url=)((http|ftp|https|ftps|irc):\/\/(www\.)?bt-star\.(ru|org|com)(:8[0-3])?([^<>\s]*))>([^<]+)<\/a>/i",
    "<a href=\\2>\\8</a>", $s);
}
...
function get_user_class_name($class) {
  ...
  switch ($class) {
	...
	case UC_GOD: return "Создатель трекера";
	
	case UC_BOT: return "Бот";
  }
  return "";
}
