...
function image_resize($mime, $source, $dest, $max_width, $max_height)
{
	$newwidth=$max_width;
...
function dbconn($autoclean = false, $lightmode = false) {
        global $mysql_host, $mysql_user, $mysql_pass, $mysql_db, $mysql_charset, $no_login;
...
if (ereg("^[a-zA-Z0-9_\.-]+@[a-zA-Z0-9_\.-]+\.[a-zA-Z]{2,7}$", $email)) 
return true;
return false;
}
...
function stdhead($title = "", $blockhide = none, $msgalert = true, $description='', $keywords='') {  
        ...
        require_once("themes/TBDev/template.php");
        require_once("themes/TBDev/stdhead.php");

} // stdhead

function stdfoot($blockhide = none) {  
global $CURUSER, $ss_uri, $tracker_lang, $queries, $tstart, $query_stat, $querytime, $show_ad;
if ($CURUSER) {
...
function loggedinorreturn($bott = false) {
        global $CURUSER, $DEFAULTBASEURL, $show_ad, $refid;
        if (!$CURUSER) {
				if($bott == false) $show_ad = false;
				if($bott&&is_bot()) {
					$CURUSER['class']=UC_BOT;
					$CURUSER['username']=$botname;
					$CURUSER['id']=1836;
					$CURUSER['ip']=getip();
					$CURUSER['adult']=1;
					user_session(); return; }
				
				if($bott&&!empty($_COOKIE['return'])&&urldecode($_COOKIE['return'])==$_SERVER['REQUEST_URI'])
				{
					$refid = -1; 
					return;}
...
$tags .= "<a style=\"font-weight:normal;\" href=\"browse.php?tag=".$tag."&incldead=1&cat=0\">".$tag."</a>, ";
if ($tags)
